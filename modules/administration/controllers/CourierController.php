...
class CourierController extends \yii\web\Controller
{
    ...
    public function actionIndex()
    {
        ...
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams, 'сouriers');
    ...
    public function actionBlackList($profile_id){
        ...
        {
            Yii::$app->session->setFlash('success', 'Курьер поставлен в чёрный список');
    ...
    public function actionUnBlackList($profile_id){
        ...
        {
            Yii::$app->session->setFlash('success', 'Курьер разблокирован в чёрный список');
            return $this->redirect(Yii::$app->request->referrer);
        }else{
            Yii::$app->session->setFlash('error', 'Ошибка! Причина уже в список или не сущ');
            return $this->redirect(Yii::$app->request->referrer);
    ...
    public function actionView($id)
    {
        ...
        if( !Yii::$app->user->can('super_admin')){
            if(!\app\models\config\Ruxsatnoma::isAgentOrCourier($model))
            {
...
//        }
    ...
    public function actionDelete($id)
    {
        ...
        if(\app\models\config\UserActions::userDelete($model)){
            Yii::$app->session->setFlash('success', "Курьер успешно удалён");
            return $this->redirect(['index']);
        }else{
